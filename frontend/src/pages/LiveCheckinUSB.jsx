...
export default function LiveCheckinUSB() {
  ...
  useEffect(() => {
    const loadDevices = async () => {
      try {
        ...
        setSelectedDevice(external?.deviceId || cams[0]?.deviceId || '')
      } catch {
  ...
  const startCamera = async () => {
    try {
      ...
      toast.error('Cannot access selected camera.')
    }
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900">📷 USB External Webcam</h1>
          <p className="text-sm text-gray-500">Automatically prefers external USB camera over built-in</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          {/* Device selector */}
          <div className="mb-4">
            <label className="block text-xs text-gray-500 mb-1">Select camera</label>
            <select value={selectedDevice} onChange={e => { setSelectedDevice(e.target.value); if (cameraOn) stopCamera() }}
              ...
              {devices.map(d => (
                <option key={d.deviceId} value={d.deviceId}>
                  ...
                  {!d.label.toLowerCase().includes('integrated') &&
                   ...
                   !d.label.toLowerCase().includes('facetime') ? ' ⭐ External' : ''}
                </option>
              ))}
            </select>
            <p className="text-xs text-gray-400 mt-1">{devices.length} camera(s) detected — external cameras marked with ⭐</p>
          </div>
          ...
          <div className="flex gap-2">
            {!cameraOn ? (
              <button onClick={startCamera} disabled={!selectedDevice}
              ...
              <>
                ...
                <button onClick={stopCamera} className="px-4 rounded-lg text-sm text-red-500 border border-red-100 hover:bg-red-50 transition-colors">Stop</button>
              </>
            )}
          </div>
